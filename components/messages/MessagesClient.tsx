  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`messages:${currentUserId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload) => {
        const row = payload.new as { id: number; interest_liker_id: string; interest_liked_id: string; sender_id: string; body: string; read_at: string | null; created_at: string; };
        const id = `${row.interest_liker_id}:${row.interest_liked_id}`;
        const message: ChatMessage = { id: String(row.id), conversationId: id, sender: row.sender_id === currentUserId ? "me" : "them", senderId: row.sender_id, text: row.body, time: formatTime(row.created_at), createdAt: row.created_at, seen: Boolean(row.read_at) };
...
function MessageBubble({ message, avatar }: { message: ChatMessage; avatar: string; }) { const mine = message.sender === "me"; return <div className={`flex items-end gap-3 ${mine ? "justify-end" : "justify-start"}`}>{!mine ? <span className="relative size-9 shrink-0 overflow-hidden rounded-full"><ProfileImage src={avatar} alt="" fill sizes="36px" className="object-cover" /></span> : null}<div className={`max-w-[82%] whitespace-pre-line rounded-[20px] px-4 py-3 text-[14px] leading-[1.45] shadow-[0_5px_18px_rgba(63,38,110,.06)] md:max-w-[68%] ${mine ? "bg-[#f0e5ff] text-[#0f1419]" : "bg-white text-[#111b21]"}`}><p>{message.text}</p><p className={`mt-1 flex items-center justify-end gap-1 text-[10px] ${mine ? "text-[#687684]" : "text-[#667781]"}`}>{message.time}{message.seen ? <CheckCheck size={14} className={mine ? "text-[#8b3de8]" : "text-[#1d9bf0]"} /> : null}</p></div></div>; }
