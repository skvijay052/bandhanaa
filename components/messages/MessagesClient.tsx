...
import { MessageCreditBalance } from "./MessageCreditBalance";
...
export function MessagesClient({ initialConversations, initialMessages, currentUserId, viewerName, avatarUrl, readReceipts }: { initialConversations: Conversation[]; initialMessages: ChatMessage[]; currentUserId: string; viewerName: string; avatarUrl: string; readReceipts: boolean; }) {
  ...
  async function sendMessage() {
    ...
    const fingerprint = JSON.stringify([conversation.id, text, reply?.id ?? null]);
    const request = retryRef.current?.fingerprint === fingerprint
      ? retryRef.current
    ...
    try {
      const supabase = createClient();
      const result = await supabase.rpc("send_message_with_credits", {
        conversation_liker_id: conversation.interestLikerId,
        conversation_liked_id: conversation.interestLikedId,
        message_body: text,
        request_id: request.id,
        reply_message_id: reply?.id ?? null,
      }).single();
      if (result.error) {
        setNotice(result.error.message.includes("Insufficient message credits")
          ? "You have no message credits left. Invite a friend to earn 10 more."
          : result.error.code === "42501"
            ? "This conversation is unavailable. An accepted connection is required."
            : "Message could not be sent. Please try again.");
        return;
      }
      const row = result.data as { id: number; body: string; read_at: string | null; created_at: string; reply_to_id?: number | null; };
      const message: ChatMessage = { id: String(row.id), conversationId: conversation.id, sender: "me", senderId: currentUserId, text: row.body, time: formatTime(row.created_at), createdAt: row.created_at, seen: Boolean(row.read_at), replyToId: row.reply_to_id ? String(row.reply_to_id) : (reply?.id ?? null) };
      setChat((items) => items.some((item) => item.id === message.id) ? items : [...items, message]);
      setConversations((items) => items.map((item) => item.id === conversation.id ? { ...item, preview: message.text, time: message.time } : item));
      setDraft((current) => current.trim() === text ? "" : current);
      setReplyingTo((current) => current?.id === reply?.id ? null : current);
      retryRef.current = null;
      window.dispatchEvent(new Event("bandhanaa-message-sent"));
    } catch { setNotice("Check your connection and try sending again."); }
    finally { sendingRef.current = false; setSending(false); }
  }

  return <div className="h-dvh overflow-hidden bg-white"><div className="app-shell"><AppSidebar active="Messages" hideMobileNavigation={mobileChatOpen} /><div className="grid min-w-0 flex-1 md:grid-cols-[390px_minmax(0,1fr)] xl:grid-cols-[430px_minmax(0,1fr)]"><ConversationPanel creditUserId={currentUserId} filter={filter} setFilter={setFilter} query={query} setQuery={setQuery} visible={visible} selectedId={selectedId} setSelectedId={(id) => { setSelectedId(id); setMobileChatOpen(true); }} mobileChatOpen={mobileChatOpen} unread={unread} viewerName={viewerName} />{selected ? <ChatPanel creditUserId={currentUserId} sending={sending} selected={selected} chat={selectedMessages} draft={draft} setDraft={setDraft} sendMessage={sendMessage} notice={notice} mobileOpen={mobileChatOpen} onBack={() => setMobileChatOpen(false)} onDetails={() => setDetailsOpen((open) => !open)} replyingTo={replyingTo} onReply={setReplyingTo} onCancelReply={() => setReplyingTo(null)} /> : <EmptyChat />}{selected && detailsOpen ? <ProfileDetails conversation={selected} onClose={() => setDetailsOpen(false)} /> : null}</div></div></div>;
}

function ConversationPanel({ creditUserId, filter, setFilter, query, setQuery, visible, selectedId, setSelectedId, mobileChatOpen, unread, viewerName }: { creditUserId: string; filter: ConversationFilter; setFilter: (value: ConversationFilter) => void; query: string; setQuery: (value: string) => void; visible: Conversation[]; selectedId: string; setSelectedId: (id: string) => void; mobileChatOpen: boolean; unread: number; viewerName: string; }) {
  return <section className={`relative flex min-h-0 min-w-0 flex-col overflow-hidden border-r border-[#dbdfe4] bg-white max-md:bg-[#fbfcff] ${mobileChatOpen ? "max-md:hidden" : ""}`}>
    ...
    <div className="relative shrink-0 px-7 pb-4 pt-7 max-md:px-4 max-md:pb-3 max-md:pt-5">
      ...
      {!mobileChatOpen ? <div className="mt-3"><MessageCreditBalance userId={creditUserId} /></div> : null}
