...
export function ProfileCard({
  ...
  return (
    <article className="group relative aspect-[3/4] min-w-0 overflow-hidden rounded-[18px] bg-[#dfe5e9] shadow-[0_5px_18px_rgba(15,20,25,.10)] ring-1 ring-black/5 transition duration-300 hover:-translate-y-1 hover:shadow-[0_16px_34px_rgba(15,20,25,.18)]">
      ...
      <span
        className={`absolute left-3 top-3 inline-flex h-7 items-center gap-1.5 rounded-full px-3 text-[11px] font-semibold text-white shadow-sm backdrop-blur-md ${profile.online ? "bg-emerald-600/90" : "bg-[#1d9bf0]/90"}`}
      >
        {profile.online ? (
          <span
        ...
        {profile.online ? "Online" : "Verified"}
      </span>
      ...
      <button
        ...
        aria-pressed={liked}
        className={`absolute right-3 top-3 z-10 grid size-9 place-items-center rounded-full border shadow-sm backdrop-blur-md transition hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white ${liked ? "border-white bg-white text-[#ff2f92] hover:bg-white" : "border-white/25 bg-black/25 text-white hover:bg-black/40"}`}
      >
        <Heart
          size={20}
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 px-4 pb-4 pt-20 text-white">
